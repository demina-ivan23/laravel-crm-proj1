    ...
    <div class="card-body">
        <ul>
            ...
            @if (!$product->deleted_at)
                @can('delete', $product)
                    <li>
                        <form action="{{ route('user.products.delete', ['product' => $product]) }}" method="POST">
                            @csrf
                            @method('DELETE')
            ...
            @else
                @can('restore', $product)
                    <li>
                        <form action="{{ route('user.products.restore', ['product' => $product]) }}" method="POST">
                            @csrf
                            @method('PUT')
            ...
            @can('forceDelete', $product)
                <li>
                    <form action="{{ route('user.products.destroy', ['product' => $product]) }}" method="POST">
                        @csrf
                        @method('DELETE')
